    ...
    for (const spec of specs || []) {
      ...
      if (
        ...
        if (type === "Rectangle") {
          ...
          const fill = colorToCss(spec.color);
          // Compute tight bounding box of the actual sector shape,
          // matching skia's computeTightBounds(). The arc sweeps from
          // angle=0 (right) counterclockwise by `angle` degrees.
          const θ = (angle * Math.PI) / 180;
          const endX = radius * Math.cos(-θ);
          const endY = radius * Math.sin(-θ);
          const bxs = [0, radius, endX];
          const bys = [0, 0, endY];
          if (angle >= 90)  bys.push(-radius); // arc passes through top
          if (angle >= 180) bxs.push(-radius); // arc passes through left
          if (angle >= 270) bys.push(radius);  // arc passes through bottom
          ...
          // Circle center (0,0 in arc coords) expressed relative to bbox center
    ...
    for (const spec of specs || []) {
      ...
      if (
        ...
        if (type === "Rectangle") {
          ...
          const angle = Number(spec.angle) || 0;
          const fill = colorToCss(spec.color);
          // Tight bounding box — same logic as canvas renderer
          const θ = (angle * Math.PI) / 180;
          const endX = radius * Math.cos(-θ);
          const endY = radius * Math.sin(-θ);
          const bxs = [0, radius, endX];
          ...
          if (angle >= 360) {
            ...
            svg =
              `<g transform="translate(${-bboxCX},${-bboxCY})">` +
              `<path d="M 0 0 L ${radius} 0 A ${radius} ${radius} 0 ${largeArcFlag} 0 ${endX} ${endY} Z" fill="${fill}"/>` +
              `</g>`;
          }
        } else if (type === "Triangle") {
          const side1 = Math.max(0, Number(spec.side1) || 0);
